---
title: "Run progress - Clay Docs"
source: "https://university.clay.com/docs/run-progress"
author:
published:
created: 2026-07-14
...
- A breakdown of rows by status:
	- 🟢 **`Successful`** — The cell completed successfully.
		- *Note: "Run condition not met" is also treated as a successful run*
...
Hovering over the progress bar displays a detailed status breakdown. This includes *all* rows in the table, even if filters or row limits are applied.
...
- The percentage of **all enrichment cells** in the table that have run.
	- *Note: Includes non-visible rows, but not non-visible columns.*
