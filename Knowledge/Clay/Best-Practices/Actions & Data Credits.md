...
published:
created: 2026-07-14
description: "Learn about credits, the virtual currency system used for running actions in Clay."
tags:
  - "clippings"
---
Overview
...
- Send Table Data (moving data between Clay tables).
- Lookup Single Row in Other Table (referencing data across Clay tables).
...
*This feature is for users on the Enterprise Plan.*
...
*Note: Your rollover and renewal happen on the same day and at the same time you originally subscribed (this timestamp is now shown in your billing details). Processing may take a few hours after that timestamp, so there can be a delay before you see Data Credits updated.*
...
### Do Send Table Data and Lookup Single Row operations count as actions?

No. Moving or referencing data between Clay tables using **Send Table Data** or **Lookup Single Row in Other Table** does not consume actions. These are internal table operations that organize your data within Clay.

**Actions are only consumed when:**

- Enriching data with external providers
- Using AI to generate or analyze content
- Executing GTM work (sending emails, syncing to CRM, exporting to external tools)

**Table operations that don't consume actions:**

- Send Table Data (moving rows between tables)
- Lookup Single Row in Other Table (referencing data)
- Lookup Multiple Rows in Other Table (referencing data)
- Clay formulas and filters

Table of contents[Quick comparison](#quick-comparison)[How Actions and Data Credits work together](#how-actions-and-data-credits-work-together)

[

Understanding Actions

](#understanding-actions)[What are Actions?](#what-are-actions)[What consumes Actions?](#what-consumes-actions)[How many Actions do I need?](#how-many-actions-do-i-need)[How to get more Actions](#how-to-get-more-actions)

[

Understanding Data Credits

](#understanding-data-credits)[What are Data Credits?](#what-are-data-credits)[Using your own API keys](#using-your-own-api-keys)[How many Data Credits do I need?](#how-many-data-credits-do-i-need)[Example: Data Credits in practice](#example-data-credits-in-practice)[Data Credits for AI](#data-credits-for-ai)

[

Action and Data Credit usage

](#action-and-data-credit-usage)[Workbook credit limits (Enterprise only)](#workbook-credit-limits-enterprise-only)

[

Rollover Data Credits

](#rollover-data-credits)[Actions: No rollover](#actions-no-rollover)[Data Credits: Yes, with limits](#data-credits-yes-with-limits)

[

Managing your plan and credits

](#managing-your-plan-and-credits)[Upgrade your plan or add more credits](#upgrade-your-plan-or-add-more-credits)[Purchase one-time Data Credit top-ups](#purchase-one-time-data-credit-top-ups)[Downgrade or cancel your plan](#downgrade-or-cancel-your-plan)

[

Cost optimization tips

](#cost-optimization-tips)[Use your own API keys](#use-your-own-api-keys)[Filter before enriching](#filter-before-enriching)[Use conditional logic](#use-conditional-logic)[Test on small batches first](#test-on-small-batches-first)

[

FAQs

](#faqs)[What's the difference between Actions and Data Credits?](#whats-the-difference-between-actions-and-data-credits)[Why do Data Credits cost different amounts?](#why-do-data-credits-cost-different-amounts)[Why do I pay Actions even when using my own API key?](#why-do-i-pay-actions-even-when-using-my-own-api-key)[What happens if I run out of Data Credits but not Actions?](#what-happens-if-i-run-out-of-data-credits-but-not-actions)[What happens if I run out of Actions but not Data Credits?](#what-happens-if-i-run-out-of-actions-but-not-data-credits)[Why can't I top up Actions?](#why-cant-i-top-up-actions)[Do unused Data Credits or Actions roll over?](#do-unused-data-credits-or-actions-roll-over)[How do I estimate costs before running an enrichment?](#how-do-i-estimate-costs-before-running-an-enrichment)[What happens to my Data Credits if I get invalid data?](#what-happens-to-my-data-credits-if-i-get-invalid-data)[How long does it take for Data Credits to renew?](#how-long-does-it-take-for-data-credits-to-renew)[Why are Data Credits being deducted unexpectedly?](#why-are-data-credits-being-deducted-unexpectedly)[What happens when I downgrade my plan?](#what-happens-when-i-downgrade-my-plan)[I have a trial, when do the Data Credits expire?](#i-have-a-trial-when-do-the-data-credits-expire)[Does CSV export consume an Action?](#does-csv-export-consume-an-action)[Do Send Table Data and Lookup Single Row operations count as actions?](#do-send-table-data-and-lookup-single-row-operations-count-as-actions)